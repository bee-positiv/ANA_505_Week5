...
# View the first few rows of the WorldCup dataset
head(WorldCup)

# Get a summary of the WorldCup dataset
summary(WorldCup)

# View the structure of the WorldCup dataset
str(WorldCup)
...
write.csv(mysample, "WorldCupSample.csv", row.names = FALSE)
...
#Revised code:
# Using piping to perform data transformations and summarizations
mysample %>%
  arrange(date) %>%                         # Arrange data by the 'date' column
  filter(spi1 < 80) %>%                     # Filter rows where 'spi1' is less than 80
  rename(Index1 = spi1, Index2 = spi2) %>%  # Rename 'spi1' to 'Index1' and 'spi2' to 'Index2'
  select(Index1, Index2, team1, team2) %>%  # Select columns 'Index1', 'Index2', 'team1', and 'team2'
  summary() %>%                             # Get a summary of the resulting dataset
